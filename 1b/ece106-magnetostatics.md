...
In general, solve problems using this law by:

1. Picking a coordinate system
2. Looking for symmetry that cancels out the magnetic field in many directions
3. Find $d\vec{l}$ or $\vec{v}$ and apply the corresponding $d\vec{B}$.
4. Use the definition of $\vec{B}=\int d\vec{B}$ such that $\vec{B}\_x=\int dB\_x$, $\vec{B}\_y=\int dB\_y$, $\vec{B}\_z=\int dB\_z$.
...
This law is used to calculate the magnetic field from current enclosed in a loop.

{% theorem(ref="Ampere's Law") %}
The magnetic field around a closed _Amperian loop_ is proportional to the current enclosed by that loop.

$$\oint \vec{B}\cdot d\vec{l}=\mu_0 I_\text{enclosed}$$
{% end %}

It is very likely that the current will be given in the form of $J$, such that $I=\int \vec{J}\cdot d\vec{S}$ and $J=J_{o\hat{r}}$

Steps to apply:

1. Apply symmetry and sketch magnetic field lines. 
...
{% theorem(ref="Faraday's Law for Solenoids") %}
Given a solenoid with $N$ turns, we may state that:

$$V_\text{ind} = \oint \vec{E}\cdot d\vec{l} = -N \frac{d\Phi_m}{dt}=-\frac{d}{dt}\Lambda$$

where $\Lambda=N\Phi_m$.
{% end %}
...
If current varies with time, then $\Lambda \propto I$. We may introduce a proportionality constant called "inductance", $L$, such that $L=\frac{\Phi_m}{I} = \frac{\Lambda}{I}$.
